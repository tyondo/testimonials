...
namespace Tyondo\Testimonial\Commands;
...
class Install extends BaseCommand
{
     ...
     * @var string
     */
    protected $signature = 'testimonial:install {--y|y : Skip question?} {--views : Also publish tyondo testimonial views.} {--f|force : Overwrite existing files.}';
     ...
     * @var string
     */
    protected $description = 'Tyondo testimonial install wizard';
    ...
    public function handle()
    {
            ...
            $this->comment(PHP_EOL.'Welcome to the Tyondo testimonial Install Wizard! You\'ll be up and running in no time...');

            try {
                $this->comment(PHP_EOL.'Publishing assets...');
                // Publish config files...
                Artisan::call('testimonial:publish:config', [
                    '--y' => true,
                    '--force' => true,
                ]);
                $this->progress(1);
                ...
                Artisan::call('testimonial:publish:assets', [
                    '--y' => true,
                    '--force' => true,
                ]);
                $this->progress(2);
                ...
                Artisan::call('testimonial:publish:packages', [
                    '--y' => true,
                    '--force' => false,
                ]);
                $this->progress(3);
                ...
                // Publish view files...
                if ($withViews) {
                    Artisan::call('testimonial:publish:views', [
                        '--y' => true,
                        '--force' => $force,
                    ]);
                }
                ...
                $this->info(PHP_EOL.'Adding testimonial routes to routes/web.php');
                $this->progress(5);
                File::append(
                    base_path('routes/web.php'),
                    "\n\nRoute::group(['prefix' => 'tyondo'], function () {\n    TyondoTestimonial::routes();\n});\n"
                );
